...
export interface PipelineOptions {
  /**
   * Optional LLM enrichment for community clusters (fork-only feature). Disabled
   * by default so analyze remains fully local and deterministic unless explicitly
   * enabled. Consumed by the `communities` phase via `ctx.options.clusterEnrichment`.
   */
  clusterEnrichment?: {
    enabled: boolean;
    llmClient: LLMClient;
